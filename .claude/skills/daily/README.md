...
- `--date YYYY-MM-DD` specifies which date to generate for. Defaults to today.
...
After gathering data, the skill synthesizes everything into a coherent narrative following the template's structure. If a character is specified, the character's output style shapes the writing voice. The finished digest is written to `content/digests/YYYY/MM/DD/daily.md` with YAML frontmatter recording the date, template name, character (if any), cadence, and which data sources were consulted. The same content is also rendered to the terminal.
